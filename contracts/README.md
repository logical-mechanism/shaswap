...
| Module | Purpose |
|---|---|
| [`types.ak`](lib/shaswap/types.ak) | Datum/redeemer encodings. `OrderDatum` (7 fields) and `PoolDatum` (6 fields) are deliberately distinct shapes so an order can never be relabelled as the pool (§5.4). |
