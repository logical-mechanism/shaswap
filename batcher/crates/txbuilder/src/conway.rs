...
use pallas_primitives::{
    ...
    Fragment, NonEmptySet, PositiveCoin, RewardAccount,
};
...
impl BuildConway for StagingTransaction {
    fn build_conway_raw(self) -> Result<BuiltTransaction, TxBuilderError> {
        ...
        // Withdrawals (ShaSwap fork): canonical reward-account -> coin map, plus the
        // sorted account list the `Reward` redeemer indexes into.
        let withdrawals_in = self.withdrawals.clone().unwrap_or_default();
        let withdrawals = if withdrawals_in.is_empty() {
            None
        } else {
            Some(
                withdrawals_in
                    .iter()
                    .map(|(acct, coin)| (RewardAccount::from(acct.clone()), *coin))
                    .collect::<std::collections::BTreeMap<RewardAccount, u64>>(),
            )
        };
        let sorted_withdrawal_accounts: Vec<Vec<u8>> = withdrawals
            .as_ref()
            .map(|m| m.keys().map(|k| (k.as_ref() as &[u8]).to_vec()).collect())
            .unwrap_or_default();
        ...
        if let Some(rdmrs) = self.redeemers {
            for (purpose, (pd, ex_units)) in rdmrs.deref().iter() {
                ...
                match purpose {
                    ...
                    // ShaSwap fork: reward (withdrawal) redeemer — index = the
                    // account's position in the canonical withdrawals order.
                    RedeemerPurpose::Reward(account) => {
                        let index = sorted_withdrawal_accounts
                            .iter()
                    ...
                    } // cert redeemers not yet supported // TODO
                }
            }
        };
        ...
        let mut pallas_tx: Tx = Tx {
            transaction_body: TransactionBody {
                ...
                withdrawals,               // ShaSwap fork: was `None // TODO`
