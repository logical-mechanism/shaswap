...
impl Serialize for RedeemerPurpose {
        ...
        S: Serializer,
    {
        let str = match self {
            ...
            RedeemerPurpose::Reward(account) => format!("withdraw:{}", hex::encode(account)),
        };
...
impl Visitor<'_> for RedeemerPurposeVisitor {
    ...
    fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        formatter.write_str(
            "'spend:{hex_txid}#{index}', 'mint:{hex_policyid}', or 'withdraw:{hex_account}'",
        )
    }
        ...
        E: de::Error,
    {
        ...
        match tag {
            ...
            "withdraw" => {
                let account = hex::decode(item)
                    .map_err(|_| E::custom("invalid withdraw redeemer account hex"))?;

                Ok(RedeemerPurpose::Reward(account))
            }
