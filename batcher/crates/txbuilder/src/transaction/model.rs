...
pub struct StagingTransaction {
    ...
    /// Withdrawals: `(reward-account bytes, amount lovelace)`. A withdraw-0 staking
    /// script uses `amount = 0` and must be paired with a `Reward` redeemer.
    /// (ShaSwap fork addition — upstream pallas-txbuilder 1.0 left this a TODO.)
    pub withdrawals: Option<Vec<(Vec<u8>, u64)>>,
...
impl StagingTransaction {
    ...
    /// Add a withdrawal of `amount` lovelace from reward `account` (raw
    /// stake-address bytes). For a withdraw-0 staking script use `amount = 0` and
    /// pair it with [`StagingTransaction::add_withdraw_redeemer`]. (ShaSwap fork.)
    pub fn withdrawal(mut self, account: Vec<u8>, amount: u64) -> Self {
        let mut w = self.withdrawals.unwrap_or_default();
        w.push((account, amount));
        self.withdrawals = Some(w);
        self
    }

    /// Remove the withdrawal targeting reward `account`.
    pub fn remove_withdrawal(mut self, account: &[u8]) -> Self {
        let mut w = self.withdrawals.unwrap_or_default();
        w.retain(|(a, _)| a.as_slice() != account);
        self.withdrawals = Some(w);
        self
    }

    /// Attach a reward (withdrawal) redeemer targeting `account` (raw stake-address
    /// bytes). Pass `ex_units = None` to fill them later from EvaluateTx. (Fork.)
    pub fn add_withdraw_redeemer(
        mut self,
        account: Vec<u8>,
        plutus_data: Vec<u8>,
        ex_units: Option<ExUnits>,
...
pub enum RedeemerPurpose {
    ...
    Mint(PolicyId),
    /// Reward (withdrawal) redeemer targeting a reward account (raw stake-address
    /// bytes). (ShaSwap fork addition — needed for withdraw-0 staking scripts.)
    Reward(Vec<u8>),
    // Cert TODO
}
