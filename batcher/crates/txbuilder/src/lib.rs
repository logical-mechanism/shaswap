...
//!
//! ## ShaSwap fork (vendored)
//!
//! This is a vendored copy of **pallas-txbuilder 1.0.0** (TxPipe, Apache-2.0) with
//! **withdrawal + reward-redeemer support added** — upstream left `withdrawals` a
//! `// TODO` (built body always set it to `None`) and `RedeemerPurpose` had no
//! `Reward` variant. ShaSwap's settlement anchor is a **withdraw-0 staking script**,
//! so it is unbuildable without this. Fork additions, all tagged "ShaSwap fork":
//! `StagingTransaction.withdrawals`, `.withdrawal()` / `.add_withdraw_redeemer()`,
//! `RedeemerPurpose::Reward`, the body `withdrawals` map + `Reward` redeemer arm
//! (index = the account's position in the canonical withdrawals order), and the
//! `RedeemerPurpose` (de)serialization for `"withdraw:{hex}"`. Everything else is
//! upstream verbatim; track upstream for unrelated fixes.
//!
